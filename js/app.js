...
Player.prototype.handleLoss = function() {
    ...
    statements.classList.add('loss-sequence')
    lossDisplay.classList.add('loss-sequence')
    statements.innerText = lossPhrases[0]
    setTimeout( () => {
        statements.classList.remove('loss-sequence')
...
Player.prototype.handleWin = function() {
    ...
    statements.classList.add('win-sequence')
    winDisplay.classList.add('win-sequence')
    statements.innerText = winPhrases[0]
    this.nextLevel()
    setTimeout( () => {
        statements.classList.remove('win-sequence')
...
Player.prototype.nextLevel = function() {
    let wins = winDisplay.innerText
    if (wins === '1') statements.innerText = 'Level: 1. Gentle stroll'
    if (wins === '5') statements.innerText = 'Level: 2. Morning rush'
    if (wins === '10') statements.innerText = 'Level: 3. Quick dash'
    if (wins === '15') statements.innerText = 'Level: 4. Run for it'
    if (wins === '20') statements.innerText = 'Level: 5. Cross fit'
    if (wins === '25') statements.innerText = 'Level: 6. Cross trial, and error'
    if (wins === '30') statements.innerText = 'Level: 7. Hit and run'
    if (wins === '35') statements.innerText = 'Level: 8. Insane mode UNLOCKED'
}
